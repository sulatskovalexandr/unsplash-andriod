...
class GetUserPhotoUseCase @Inject constructor(private val usersPhotoRepository: UserRepository) :
    UseCase<UserPhotoParam, List<UserPhoto>>(Dispatchers.IO) {

    override suspend fun execute(param: UserPhotoParam): List<UserPhoto> =
        usersPhotoRepository.getUserPhoto(param.userName, param.page)
}
